...
<main>
    <style type="text/css">
        .account_dash_home_split{
            display:grid;
            grid-template-columns:minmax(0,1fr) 360px;
            gap:16px;
            align-items:start;
        }
        .account_dash_home_main,
        .account_dash_home_side{
            min-width:0;
        }
        .account_dash_home_side{
            position:sticky;
    ...
    <div class="content account_dash_page">
        <div class="account_dash_header">
            <h1 class="account_dash_kicker">حساب من</h1>
            <p class="account_dash_title">نمای کلی حساب کاربری و دسترسی سریع</p>
            <div class="account_dash_meta">
                <div class="account_dash_meta_item">
                    <span class="account_dash_meta_label">خوش آمدید</span>
                    <span class="account_dash_meta_value"><?php echo $_SESSION["user"]->data()["name"]; ?></span>
                </div>
                <div class="account_dash_meta_item">
                    <span class="account_dash_meta_label">شماره همراه</span>
                    <span class="account_dash_meta_value"><?php echo $_SESSION["user"]->data()["tel"]; ?></span>
                </div>
                <div class="account_dash_meta_item">
                    <span class="account_dash_meta_label">پست الکترونیکی</span>
                    <span class="account_dash_meta_value"><?php echo isset($_SESSION["user"]->data()["email"]) && $_SESSION["user"]->data()["email"] ? $_SESSION["user"]->data()["email"] : "-"; ?></span>
                </div>
            </div>
        </div>
        <div class="account_dash_home_split">
            <div class="account_dash_home_main">
            <div class="account_dash_panel">
                <h3 class="account_dash_panel_title">آخرین سفارش‌ها</h3>
            ...
            <?php
                ...
                WHERE uid = ? AND state >= 1 AND del_flag = 0 ORDER BY create_date DESC LIMIT 3";
            ...
            <div class="account_dash_home_side">
            <div class="account_dash_panel">
                <h3 class="account_dash_panel_title">دسترسی‌های حساب</h3>
                <p class="account_dash_panel_text">از این بخش می‌توانید سفارش‌ها، اطلاعات حساب و سایر بخش‌های مرتبط با حساب کاربری خود را مدیریت کنید.</p>
                    <div class="account_dash_actions_box">
                <div class="account_dash_actions" style="display:grid;grid-template-columns:minmax(0,1fr);gap:10px;">
            <?php
                $st = "SELECT flag,name FROM $atb";
                $st = $mysqli->prepare($st);
                if(!$st->execute()){
                    echo "E";
                    exit;
            ...
            <?php
                while($row = $res->fetch_assoc()){ ?>
                    <a onclick="sub_show('state','<?php echo $row["flag"]; ?>')" class="btn"><?php echo $row["name"]; ?></a>
            <?php }
            ?>
