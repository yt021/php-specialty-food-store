...
<?php
    ...
    if($_SESSION[$cf]->level == 0){
        ...
        if(isset($_POST["cat_id"]) && var_exist($_POST["cat_id"],$tb,"id")){
            $_SESSION[$cf]->level = 1;
            $_SESSION[$cf]->id = $_POST["cat_id"];
        }
    ...
    if($_SESSION[$cf]->level == 1){
        if($_SESSION[$cf]->id == "new")$sub_title="دسته‌ جدید";else
        $sub_title = getVarFromDB($tb,"name","id",$_SESSION[$cf]->id);
