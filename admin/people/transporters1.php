<?php
    if(isset($indexed)){
        if($indexed == 1){?>
        <a class="btn" onclick="sub_show('id','new')">ارسال‌کننده جدید</a>
        </div>  
    </div>
...
if($_SESSION[$cf]->del_flag == 1)echo "<h3>حذف شده ها</h3>";
    ...
    echo '
            ...
            
        {
            ...
            $st = "SELECT id,name,create_date,show_title FROM $tb WHERE del_flag = $del_flag $order_str";
            ...
            while($row = $res->fetch_assoc())
            {
                ...
                if($id == 1){
                    $st = "SELECT count(id) as TV FROM orders WHERE CHAR_LENGTH(post_ref_id) = 24 AND del_flag = 0";
                }else{
                    $st = "SELECT count(id) as TV FROM orders WHERE post_ref_id = $id AND del_flag = 0";
                }
                $st = $mysqli->prepare($st);
                $st->execute();
                $res2 = $st->get_result();
