<?php
    if(isset($indexed)){
        if($indexed == 1){?>
        <a class="btn" onclick="sub_show('new_post','new')">مطلب جدید</a>
        </div>  
    </div>
...
<?php
    echo '
            ...
            
        {
            ...
            $del_flag = 0;
            $st = "SELECT id,name,title,category,keywords,description,last_update FROM $tb WHERE del_flag = $del_flag AND section = '$section' $order_str";
            ...
            while($row = $res->fetch_assoc())
            {
                ...
                echo "<tr>
                        ...
                        <td ".'onclick ="sub_show('."'post_id','".$id."'".')" >
