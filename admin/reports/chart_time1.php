...
<?php
    $fields = array(
        "amount"=>"مقدار",
        "percentage"=>"درصد",
        "per_range"=>"استاندارد شده (به فروش بازه)"
    );
    foreach($fields as $key=>$f){
        $checked = "";
...
if($_SESSION[$cf]->sql_where["type"] == "percentage"){
    ...
    while($row = $res->fetch_assoc())
    {
        ...
        if($order->oid != $oid){
            ...
            foreach($od_fields as $f){
                $total->amount[$f] += $order->values[$f];
            }
            $total->amount["clients"]->add_item($order->uid);
            $total->amount["orders"]->add_item($order->oid);
    ...
    $total->amount["clients"] = substr_count($total->amount["clients"]->order_str("users"),",");
    $total->amount["orders"] = substr_count($total->amount["orders"]->order_str("users"),",") - 1;
...
//                    echo $total->amount[$f]."<br>";
                    $chart_data[$i]->percentage[$f] = 100*$chart_data[$i]->amount[$f]/$total->amount[$f];
                    
                    if($max->amount[$f]<$chart_data[$i]->amount[$f])$max->amount[$f]=$chart_data[$i]->amount[$f];
                }
            }
            if($total->amount[$f])$max->percentage[$f] = 100*$max->amount[$f]/$total->amount[$f];
        }
    }
//    echo $time_where;
}
...
if($_SESSION[$cf]->sql_where["type"]=="per_range"){
    ...
    foreach($ranges as $range){
        ...
        while($row = $res->fetch_assoc())
        {
            ...
            if($order->oid != $oid){
                ...
                foreach($od_fields as $f){
                    $max_data[$k]->amount[$f] += $order->values[$f];
                }
                $max_data[$k]->amount["clients"]->add_item($order->uid);
                $max_data[$k]->amount["orders"]->add_item($order->oid);
        ...
        $max_data[$k]->amount["clients"] = substr_count($max_data[$k]->amount["clients"]->order_str("users"),",");
        $max_data[$k]->amount["orders"] = substr_count($max_data[$k]->amount["orders"]->order_str("users"),",") - 1;
        $k++;
    ...
    foreach($bar_fields as $f){
        if($_SESSION[$cf]->sql_where["items"][$f] == 1){
            for($i=1;$i<$k;$i++){
                if($max_data[$i]->amount[$f]){
                    $chart_data[$i]->percentage[$f] = 100*$chart_data[$i]->amount[$f]/$max_data[$i]->amount[$f];
                    
                    if($max->amount[$f]<$chart_data[$i]->amount[$f])$max->amount[$f]=$chart_data[$i]->amount[$f];
                    if($max->percentage[$f]<$chart_data[$i]->percentage[$f])$max->percentage[$f]=$chart_data[$i]->percentage[$f];
                }
            }
        }
    }
...
<?php
    ...
    for($i=1;$i<$k;$i++){
        $height = 0;
        if($max->percentage[$f])
        $height = (100/$ratio*$chart_data[$i]->percentage[$f]/$max->percentage[$f]);
...
<style type="text/css">
    div.chart_div div.chart.line ul{
        width:100%;
        height:100%;
        position:relative;
    }
    div.chart_div div.chart.line ul li{
        direction:ltr;
        position:absolute;
        bottom:calc(var(--y) * 1px);
        left:calc(var(--x) * 1px);
        display:table;
    }
    div.chart_div div.chart.line ul li div.data-point{
        width:15px;
        height:15px;
        border-radius:100%;
    ...
    div.chart_div div.chart.line ul li div.line-segment{
        
        width: calc(var(--hypo) * 1px);
        height:5px;
    ...
    div.chart_div div.horizontal ul li{
        width: min(calc(var(--iwidth) * 1px) , 25px);
    }
    div.chart_div div.horizontal ul li span{
        height:min(calc(var(--iwidth) * 1px) , 25px);
        font-size:max(min(calc(var(--iwidth) * 1px - 2px) , 14px) , 6px);
    }
