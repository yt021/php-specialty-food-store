...
<?php
        ...
        if($_SESSION[$cf]->sql_where["type"]=="percentage"){
            ...
            echo $max->percentage[$f]." - $max_chart<br>";
            if($division < 1){
...
<?php
    ...
    for($i=1;$i<$k;$i++){
        $height = 0;
        if($max->percentage[$f])
        $height = (100/$ratio*$chart_data[$k-$i]->percentage[$f]/$max->percentage[$f]);
    ...
    echo '<path d=" '.$path_str.' "  FILL="none"/> ';
    for($i=1;$i<$k;$i++){
        echo '<circle cx="'.$points[$i][0].'" cy="'.$points[$i][1].'" r="7" />';
    }
?>
