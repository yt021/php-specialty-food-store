...
<?php
    if($_SESSION[$cf]->sql_where["type"]=="percentage"){
        $max_chart = ceil($max->percentage[$f]*1.1);
        $ratio = $max_chart/$max->percentage[$f];
        $min_chart = 0;
        ...
        $max_chart = ceil($max->amount[$f]*1.1);
        $ratio = $max_chart/$max->amount[$f];
        $min_chart = 0;
        $step = ($max_chart-$min_chart)/$vertical_items_no;
        if($step == 0){
            $vertical_items_no = 1;
...
<?php
    for($i=1;$i<$k;$i++){
        echo '<li style="height:'.(100/$ratio*$chart_data[$k-$i]->percentage[$f]/$max->percentage[$f]).'%;"></li>';
    }
?>
